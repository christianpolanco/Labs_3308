...
$(document).ready(function() {
  var url ='https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=40.015,-105.27&forecast_days=6&units=f'; //Place your weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5
  ...
  var f = data.current.temperature;
  $("#temp_today").html(f + " F");
  $("#thermometer_inner").height(f+"%");
  $("#precip_today").html(data.current.precip+" IN");
  ...
  $("#summary_today").html(data.current.weather_descriptions);
    ...
    for(var i = 1; i < 6; i++){
      ...
      $("#5_day_forecast").append(` 
      
      <div style="width: 20%;">
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">`+ dayOfWeek +`</h5>
          <p class="card-text">High:` + getKey(i).maxtemp +` F <br>
            Low: ` + getKey(i).mintemp + ` F<br>
